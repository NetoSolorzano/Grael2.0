    ...
    public partial class facelect : Form
    {
        ...
        private void bt_agr_Click(object sender, EventArgs e)
        {
            if (tx_serGR.Text.Trim() != "" && tx_numGR.Text.Trim() != "" && Tx_modo.Text == "NUEVO")
            {
                ...
                if (dataGridView1.Rows.Count > int.Parse(v_mfildet))
                {
                    MessageBox.Show("Limite máximo de guías alcanzado!", " Atención ", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    tx_numGR.Text = "";
                    tx_numGR.Focus();
                    return;
                }
